<?php
	...
	$datos = $controlador->datosEnemigoModificar($_GET["variable1"]);
	
	var_dump($_POST);
	
	if(isset($_POST) && !empty($_POST)){
		$vacio=$controlador->modificarEnemigo($_POST, $_GET["variable1"]);
	}
	...
	<body>
		...
		<main>
			<div id="divConfiguracion">
				<form action="" id="formularioModificarEnemigo" method="POST" onSubmit="return confirm('¿Está seguro de querer modificar este enemigo?.')">
					<h2>Modificación</h2>
					...
					<?php
						echo 	'<input value="'.$datos[0]["nombre"].'" type="text" name="nombre"/>
								...
								<input value="'.$datos[0]["nombreImagen"].'" type="text" name="nombreImagen"/>';
					?>
